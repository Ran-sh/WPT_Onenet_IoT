...
test('网页活动资源统一标记V6.0.0', () => {
  ...
  assert.match(read('service-worker.js'), /wpt-v6-0-0-web-16/);
...
test('CSS统一入口为dashboard.css且Service Worker预缓存同步升级', () => {
  ...
  assert.match(worker, /CACHE\s*=\s*'wpt-v6-0-0-web-16'/);
});
...
test('R66 Service Worker 缓存本地 Tailwind', () => {
  const worker = read('service-worker.js');
  assert.match(worker, /CACHE\s*=\s*'wpt-v6-0-0-web-16'/, 'SW 缓存版本必须升级为 web-16');
...
test('R45 SW 同源全 network-first、导航不缓存（web-16）', () => {
  const worker = read('service-worker.js');
  assert.match(worker, /CACHE\s*=\s*'wpt-v6-0-0-web-16'/);
